...
mod tests {
    ...
    async fn test_deactivate_session() {
        ...
        let service = TableSessionService::new(repo);
        let deactivated_session = service.deactivate_session(created_session.id).await.unwrap();
    ...
    async fn test_deactivate_nonexistent_session() {
        ...
        match result {
            Err(super::TableSessionServiceError::Repository(crate::repository::TableSessionRepositoryError::Database(e))) => {
